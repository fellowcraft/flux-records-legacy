...
where solo = '1'
AND  Released <= '#DateFormat(now(),"YYYY-MM-DD")#' 
ORDER by RIGHT(Composed,4) DESC
</CFQUERY>
...
<CFOUTPUT>
#Evaluate(records.recordcount)# albums |  
...
<TD WIDTH="94" 
    ALIGN="center" 
    CLASS="style2bigger" 
    colspan=3><A HREF='album.cfm?#Replace(name," ","-","ALL")#'><STRONG>#name#</STRONG></A><SPAN CLASS="styleTiny"><cfif NameExt neq ''><br /> <br />(#NameExt#)</cfif><BR /> <BR /><i>released <CFIF DateFormat(Released,"YYYYMMDD") LTE DateFormat(now(),"YYYYMMDD")>#DateFormat(Released,"full")#<CFELSE> </CFIF></i></SPAN></TD>
...
<TD WIDTH="94" 
    ALIGN="center" 
    CLASS="style2b"><A HREF='album.cfm?#Replace(Trim(name)," ","-","ALL")#'><STRONG>#Trim(name)#</STRONG><cfif NameExt neq ''><BR />(#Left(NameExt,40)#)</cfif></A><BR /><SPAN CLASS="styleTiny">#Composed#</SPAN></TD>
...
<TD VALIGN="TOP" CLASS="style3c"><A HREF='album.cfm?#Replace(Trim(name)," ","-","ALL")#'><STRONG>#name#</STRONG><CFIF trim(NameExt) NEQ ''> (#NameExt#)</CFIF></A><BR><SPAN CLASS="style2cfade">#label#</SPAN></TD>
